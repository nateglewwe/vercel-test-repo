...
router.put('/gearAssignEvent/:id', (req, res) => {
  ...
  `;
  const setGearForEventQueryArgs = [req.params.id, req.body.eventId]
  ...
  WHERE event_id = $1;
  `;
  const nullOutPrevEventQueryArg = [req.body.eventId]
  ...
  WHERE "gear_list".id = $1;
  `;
  const assignEventQueryArgs = [req.params.id, req.body.eventId]

  pool.query(nullOutGear1Query, nullOutGear1QueryArg)
  .then(result => {
    pool.query(setGearForEventQuery, setGearForEventQueryArgs)
    .then (result => {
      pool.query(nullOutPrevEventQuery, nullOutPrevEventQueryArg)
      .then (result => {
        pool.query(assignEventQuery, assignEventQueryArgs)
        .then(result => {
